...
public class HomeController : MyBaseController
{
	...
	{		
		IndexVM model = new IndexVM()
		{
			...
			AllCookies = GetAllCookies(HttpContext.Request),
			SessionCookie = HttpContext.Request.Cookies[Literals.SessionCookieName],
			AuthCookie = HttpContext.Request.Cookies[Literals.AuthenticationCookieName],
			AuthenticationTicketFromCookie = GetAuthenticationTicketFromCookie(),
			AuthenticationTicketFromAuthResult = GetAuthenticationTicketFromAuthResult()
		};

		if (Request.Cookies.TryGetValue(Literals.SessionCookieName, out var sessionCookie))
		{
		...
		if (Request.Cookies.TryGetValue(Literals.AuthenticationCookieName, out var authCookie))
		{
			// Manually decrypting a cookie is virtually useless
